<template>
  <div class="py-4">
    <b-container fluid>
      <div class="mb-3">
        <b-button to="/admin/rooms">Back to List</b-button>
      </div>
      <b-card class="mb-3" no-body header="Room Info">
        <b-card-body>
          <div v-if="!isLoadingRoomInfo && roomInfo">
            ...
            <b-form-row class="mb-2">
              <b-col md="3"><span class="text-muted">Room Code</span></b-col>
              <b-col md="9">{{ roomInfo.code }}</b-col>
            </b-form-row>
            <b-form-row class="mb-2">
...
import { fetchRoomDetail, fetchRoom } from '@/api/rooms.js';
...
export default {
  ...
  methods: {
    async fetchRoom() {
      ...
      try {
        const roomInfo = await fetchRoom(this.$fireStore, this.roomId);
        const roomDetail = await fetchRoomDetail(this.$fireStore, this.roomId);
        this.roomInfo = { ...roomInfo, ...roomDetail };
      } catch (error) {
        console.error(error);
        globalToast(
          this.$bvToast,
    ...
    async fetchAttendants() {
      ...
      try {
        const attendants = await fetchAllAttendants(
          this.$fireStore,
          this.roomId
        );
    ...
    async fetchQuestions() {
      ...
      try {
        const questions = await fetchAllQuestions(this.$fireStore, this.roomId);
